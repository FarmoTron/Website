...
import { setStore, t, stakeTRX, withdrawTRX, unstakeTRX, connect,  listenTronLink, initTronLinkWallet} from "../../utils/utils";
...
const Tron = (props) => {
  ...
  const { modal,defaultAccount,activeItem,accountInfo,infoenergy,chainid,inProcess,emptyInfo,latestinfo,isTronConnected } = useSelector(getData);
  ...
  useEffect(() => {
    if (latestinfo) {
      setApy(latestinfo.tron.apy);
      setTvl(latestinfo.tron.trxtvl);
    }
  }, [latestinfo])
  ...
  const confirmUnstake = async () => {
    if (!inProcess) {
      setBtnTXT("Sending ...")
      await unstakeTRX(withdrawamount);
      setBtnTXT("CONFIRM")
      setState("new")
    }
  }
...
export default withRouter(Tron);
